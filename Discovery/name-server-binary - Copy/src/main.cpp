...
int main() {
    ...
    std::thread removeThread{[&registry]() {
        while (true) {
            std::this_thread::sleep_for(std::chrono::seconds(5));

            std::lock_guard<std::mutex> lock{registry.mutex};
            auto now{std::chrono::system_clock::now()};
            for (auto servicesIt{registry.services.begin()}; servicesIt != registry.services.end();) {
                auto& providers{servicesIt->second};

                for (auto providerIt{providers.begin()}; providerIt != providers.end();) {
                    auto elapsed{std::chrono::duration_cast<std::chrono::seconds>(
                        now - providerIt->lastHeartbeat)};

                    if (elapsed.count() > 10) {
                        std::cout << "Removing unresponsive provider\n";
                ...
                if (providers.empty()) {
                    servicesIt = registry.services.erase(servicesIt);
                }
                else {
                    servicesIt++;
                }
